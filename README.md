...
- 落盘字节数与响应声明的 `Content-Length` 对一遍（没有则退回目录列表里的尺寸），对不上报
  `传输不完整` 并把半截文件删掉，不留看着完整其实残缺的文件。
...
- 客户端不认 `Transfer-Encoding: chunked`（自家 `/pull` 带长度、不分块，所以无碍）。
- 客户端 v1 顺序拉取：一文件一条连接、`Connection: close`；目录内单文件失败只告警并继续，
